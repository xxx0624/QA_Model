...
version_num = '1'
trainfile = '/home/zhengxing/QAModel/corpus/ccir17/train/train.1.json_train_part_1_data_version_' + version_num
train0file = '/home/zhengxing/QAModel/corpus/ccir17/train/train.1.json_train_part_0_data_version_' + version_num
test1file = '/home/zhengxing/QAModel/corpus/ccir17/train/train.1.json_test_part_data_version_' + version_num
...
'''
def load_data(trainList, vocab, batch_size, words_num_dim):
    train_1, train_2, train_3 = [], [], []
    mask_1, mask_2, mask_3 = [], [], []
    counter = 0
    while True:
...
def validation(validate_model, testList, vocab, batch_size, words_num_dim):
    ...
    while True:
        ...
        if index >= len(testList):
            break
        #log('Evaluation ' + str(index), logfile_path)
    ...
    for qid, items in sdict.items():
        ...
        log('qid = ' + str(qid) + ' / dcg3 = ' + str(DCG(label_list, top_k_3)) + ' / idcg3 = ' + str(idcg3), logfile_path)
        log('qid = ' + str(qid) + ' / dcg5 = ' + str(DCG(label_list, top_k_5)) + ' / idcg5 = ' + str(idcg5), logfile_path)
        log('qid = ' + str(qid) + ' / dcg_all = ' + str(DCG(label_list, top_k_all)) + ' / idcg_all' + str(idcg_all), logfile_path)
    log('ALL-DCG: all-dcg3 = ' + str(dcg3) + ' / ' + 'all-dcg5 = ' + str(dcg5) + ' / ' + 'all-dcg = ' + str(dcg_all), logfile_path)
    log('ALL-iDCG: all-idcg3 = ' + str(idcg3) + ' / ' + 'all-idcg5 = ' + str(idcg5) + ' / ' + 'all-idcg = ' + str(idcg_all), logfile_path)
    log('AVG-DCG3 = ' + str(dcg3 / idcg3) + ' / AVG-DCG5 = '+ str(dcg5 / idcg5) + ' / AVG-DCG = ' + str(dcg_all / idcg_all), logfile_path)
    #log('TOP: top1 = ' + str(top_1 / qa_count) + ' / ' + 'top3 = ' + str(top_3 / qa_count), logfile_path)
