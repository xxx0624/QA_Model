...
#dataset
version_num = '2-10-all-just-url-pure'
file_name = 'json.longshort.new.all'
trainfile = '/home/zhengxing/QAModel/corpus/ccir17/train/' + file_name + '_train_part_1_data_version_' + version_num
train0file = '/home/zhengxing/QAModel/corpus/ccir17/train/' + file_name + '_train_part_0_data_version_' + version_num
test1file = '/home/zhengxing/QAModel/corpus/ccir17/test/' + file_name + '_test_part_data_version_' + version_num
vectorsfile = '/home/zhengxing/exp_data/word_vector_cn/baike_vector.bin'
#vectorsfile = '/opt/exp_data/word_vector_cn/all_dbqa.bin'

#log file
logfile_path = '/home/zhengxing/QAModel/log/ccir'
...
def DCG(label_list, top_k):
    sum = float(0)
    for index in range(0, top_k):
        sum += (1.0 * math.pow(2, label_list[index]) - 1.0) / (math.log(index + 2, 2) * 1.0)
    return sum


def passage_json(passage_id, rank):
    return "{\"passage_id\": " + passage_id + ", \"rank\": " + rank + "}";
...
def validation(validate_model, testList, vocab, batch_size, words_num_dim):
    global logfile_path
    result_log_file = logfile_path + '.result'
    ...
    while True:
        ...
        if len(score_list) >= len(testList):
            break
    fw = codecs.open(result_log_file, 'w', encoding='utf-8')
    pre_query_id = None
    pre_query = None
    to_write_str = ''
    passage_list_rank = []
    ...
    for score in score_list:
        print 'Generate evaluation ......', index
        query_id = testList[index][1].strip().split(':')[1].split('_')[0]
        passage_id = testList[index][1].strip().split(':')[1].split('_')[1]
        query = testList[index][4].strip()
        if pre_query_id is None:
            pre_query_id = query_id
            pre_query = query
            passage_list_rank.append((passage_id, float(score)))
            clear_or_not = False
        elif pre_query_id == query_id:
            pre_query_id = query_id
            pre_query = query
            passage_list_rank.append((passage_id, float(score)))
            clear_or_not = False
            ...
            #save previous
            passage_list_rank.sort(cmp=comp1)
            for pg_rank in range(len(passage_list_rank)):
                if pg_rank == 0:
                    to_write_str += passage_json(passage_list_rank[pg_rank][0], str(pg_rank))
                else:
                    to_write_str += ',' + passage_json(passage_list_rank[pg_rank][0], str(pg_rank))
            clear_or_not = True
            to_write_str = "{" \
                                ...
                                "\"query\": \"" + pre_query + "\"," \
                                "\"ranklist\": [" + to_write_str + "]" \
                            "}"
            fw.write(to_write_str + '\n')
            fw.flush()
            #prepare now
            pre_query_id = query_id
            pre_query = query
