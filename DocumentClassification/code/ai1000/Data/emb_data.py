...
def get_tag_vector_of_dim(w2v, word, vec_dim):
    if word == UNKNOWN:
        v_list = []
        ...
        if line != '':
            ...
            if len(line) == 2:
                ...
                for i in range(len(w_list)):
                    w_vec = get_vector_of_dim(w2v, w_list[i], VEC_DIM)
                    tag_vec = get_tag_vector_of_dim(w2v_tag, tag_list[i], VEC_DIM)
                    for item in w_vec:
                        fw_w.write(str(item) + ',')
                    fw_w.write('\n')
                    for item in tag_vec:
    ...
    for line in fr:
        ...
        if line != '':
            ...
            if len(line) == 2:
                ...
                for i in range(len(w_list)):
                    w_vec = get_vector_of_dim(w2v, w_list[i], VEC_DIM)
                    tag_vec = get_vector_of_dim(w2v, tag_list[i], VEC_DIM)
                    for item in w_vec:
                        fw_w.write(str(item) + ',')
                    fw_w.write('\n')
                    for item in tag_vec:
